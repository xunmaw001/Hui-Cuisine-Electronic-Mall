    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="美食类型" prop="meishileixing">
          <el-select v-model="ruleForm.meishileixing" placeholder="请选择美食类型">
            <el-option
...
export default {
  ...
  methods: {
	...
	this.ruleForm.meishijieshao = this.ruleForm.meishijieshao.replace(reg,'../../../ssm0669t/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `meishishangcheng/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.meishishangchengCrossAddOrUpdateFlag = false;
                  this.parent.search();
